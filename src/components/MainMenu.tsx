...
export const MainMenu: FunctionalComponent<MainMenuProps> = ({
  ...
  return (
    <div className="theme-page-bg h-dvh w-full flex flex-col items-center justify-center p-6 animate-fade-in">
      <div className="theme-panel max-w-md w-full rounded-3xl shadow-2xl p-8 md:p-12">
        <div className="text-center mb-10">
          <h1 className="text-4xl md:text-5xl font-black text-balance tracking-tight mb-2">
            {copy.menuTitleMain} <span>{copy.menuTitleAccent}</span>
          </h1>
        ...
        <div className="space-y-4">
          ...
          {difficulties.map((diff) => (
            ...
            >
              <div>
                <h3 className="text-lg font-bold">{copy.difficultyLabel(diff)}</h3>
                <p className="text-sm theme-muted-text">
                  {copy.difficultyDescription(diff, progress[diff].max)}
                </p>
              </div>
          ...
          >
            <div>
              <h3 className="text-lg font-bold">{copy.difficultyLabel("Custom")}</h3>
