...
function runVitest(skipCompiler) {
  console.log(`🚀 Running tests ${skipCompiler ? 'WITHOUT' : 'WITH'} React Compiler...`);
  try {
    ...
    const output = execSync(`pnpm vitest run --reporter=json --coverage=false`, {
      stdio: ['ignore', 'pipe', 'inherit'],
      env: { ...process.env, SKIP_COMPILER: String(skipCompiler) }
    });
    ...
    if (error.stdout) {
      try {
        ...
        console.error(`❌ Failed to parse Vitest JSON output: ${parseError.message}`);
        throw error;
      }
    }
...
async function main() {
  try {
    ...
    console.log('\n📊 Comparison Results:');
    console.log('----------------------------');
    console.log(`Tests Total:   With=${withResults.numTotalTests.toString().padStart(3)} | Without=${withoutResults.numTotalTests.toString().padStart(3)}`);
    console.log(`Tests Passed:  With=${withResults.numPassedTests.toString().padStart(3)} | Without=${withoutResults.numPassedTests.toString().padStart(3)}`);
    console.log(`Tests Failed:  With=${withResults.numFailedTests.toString().padStart(3)} | Without=${withoutResults.numFailedTests.toString().padStart(3)}`);
    console.log('----------------------------');
    ...
    const getFailedNames = (results) => results.testResults
